...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            pgnode = PgNode_a(action)
            if pgnode.prenodes.issubset(self.s_levels[level]):
                action_nodes.add(pgnode)
                for s_node in self.s_levels[level]:
                    if s_node in pgnode.prenodes:
                        # s_node.children.add(pgnode)
                        # pgnode.parents.add(s_node)
                        pgnode.children.add(s_node)
                        s_node.parents.add(pgnode)
    ...
    def add_literal_level(self, level):
        ...
        s_nodes = set()
        #get previous level...then get effnodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # if use all the 4 combinations similar to interference_mutex
        # somehow the test don't pass
        for i1 in node_a1.action.effect_add:
            for i2 in node_a2.action.effect_rem:
                if i1 == i2:
                    return True

        # for i1 in node_a1.action.effect_rem:
        #     for i2 in node_a2.action.effect_add:
        #         if i1 == i2:
        #             return True

        for i1 in node_a2.action.effect_add:
            for i2 in node_a1.action.effect_rem:
                if i1 == i2:
                    return True

        # for i1 in node_a2.action.effect_rem:
        #     for i2 in node_a1.action.effect_add:
        #         if i1 == i2:
        #             return True
        # return node_a1.action.
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for i1 in node_a1.action.effect_add:
            for i2 in node_a2.action.precond_neg:
                if i1 == i2:
                    return True

        for i1 in node_a1.action.effect_rem:
            for i2 in node_a2.action.precond_pos:
                if i1 == i2:
                    return True

        for i1 in node_a2.action.effect_add:
            for i2 in node_a1.action.precond_neg:
                if i1 == i2:
                    return True

        for i1 in node_a2.action.effect_rem:
            for i2 in node_a1.action.precond_pos:
                if i1 == i2:
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # somhow this logic does not work with prenodes but have to use parent nodes
        #even function description mention precondition of one action mutex than other
        #again slack came to rescue
        for i1 in node_a1.parents:
            for i2 in node_a2.parents:
                if i1.is_mutex(i2):
                    return True

        for i1 in node_a2.parents:
            for i2 in node_a1.parents:
                if i1.is_mutex(i2):
                    return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        #negation means symbol equal but not is_pos...thanks slack
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for s_node in self.s_levels[level]:
                    #need to check for is_pos also...else test fails..similar discussion on slack
                    #also need to break from both inner for loops once
                    #at a particular s_node and level symbol matches
                    if s_node.symbol == g and s_node.is_pos == True:
                        level_sum += level
                        level_found = True
                        break
                if level_found:
